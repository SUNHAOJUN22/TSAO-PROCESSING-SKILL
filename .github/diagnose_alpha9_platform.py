...
def run(label: str, command: list[str], timeout: int = 1800) -> dict[str, object]:
    ...
    return {
        ...
        "tail": output[-6000:] if completed.returncode else output[-1000:],
    }
...
def main() -> int:
    ...
    commands = (
        (
            "compile_release_scripts",
            [python, "-m", "py_compile", ".github/apply_alpha9_release.py", ".github/fix_alpha9_release_residuals.py"],
        ),
        ("apply_alpha9", [python, ".github/apply_alpha9_release.py"]),
        ("clean_residuals", [python, ".github/fix_alpha9_release_residuals.py"]),
    ...
    if all(item["returncode"] == 0 for item in checks):
        ...
        for label, command in wheel_commands:
            result = run(label, command)
            checks.append(result)
            if result["returncode"] != 0:
                break
