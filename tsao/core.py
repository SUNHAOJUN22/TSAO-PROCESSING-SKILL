...
ALLOWED_TRANSITIONS = {
    GateStatus.NOT_EVALUATED: {GateStatus.HOLD, GateStatus.CONDITIONAL, GateStatus.PASS, GateStatus.FAIL, GateStatus.RETIRED},
    GateStatus.HOLD: {GateStatus.CONDITIONAL, GateStatus.PASS, GateStatus.FAIL, GateStatus.RETIRED},
    GateStatus.CONDITIONAL: {GateStatus.HOLD, GateStatus.PASS, GateStatus.FAIL, GateStatus.RETIRED},
...
class EvidenceItem:
    ...
    def usable(self, as_of: date | None = None) -> bool:
        ...
        expired = bool(self.review_due_on and date.fromisoformat(self.review_due_on) < as_of)
        return (
            self.grade in {"A", "B"}
...
class ModelRecord:
    ...
    def qualified(self) -> bool:
        if self.status != "QUALIFIED" or self.identifiability not in {"PASS", "CONDITIONAL"}:
            return False
        return self.risk_class not in {"MR4", "MR5"} or bool(self.independent_reviewer)
...
ROUTES = {
    "epdm": {"epdm", "epr", "ethylene propylene diene", "乙丙橡胶", "三元乙丙", "enb", "dcpd", "vnb"},
    "poe": {"poe", "polyolefin elastomer", "聚烯烃弹性体", "乙烯辛烯", "solution polyolefin elastomer"},
    "polymer-general": {"polymer", "polymerization", "聚合", "树脂", "橡胶", "分子量", "催化剂"},
    "bioprocess": {"fermentation", "bioreactor", "发酵", "enzyme", "cell culture"},
    "electrochemical": {"electrolysis", "battery", "fuel cell", "电化学", "电解", "电池"},
    "solids": {"crystallization", "precipitation", "powder", "结晶", "沉淀", "粉体"},
    "fine-chemical-batch": {"batch", "fine chemical", "pharmaceutical", "批式", "精细化工", "医药"},
    "petrochemical": {"refinery", "petrochemical", "olefin", "石化", "炼化", "裂解"},
}
...
def validate_gate_sequence(gates: list[GateRecord]) -> list[str]:
    ...
    for i in range(1, 19):
        ...
        if current and previous and current.status == GateStatus.PASS and previous.status not in {GateStatus.PASS, GateStatus.RETIRED}:
            issues.append(f"G{i} cannot PASS before G{i-1}")
    return issues


def balance_residual(inputs: dict[str, float], outputs: dict[str, float], generation: dict[str, float] | None = None) -> dict[str, float]:
    generation = generation or {}
    keys = set(inputs) | set(outputs) | set(generation)
    return {key: float(inputs.get(key, 0) + generation.get(key, 0) - outputs.get(key, 0)) for key in sorted(keys)}
...
PROJECT_DIRS = [
    "00_governance", "01_evidence", "02_requirements", "03_measurement_data", "04_chemistry",
    "05_models", "06_lab", "07_bench", "08_separation_recycle", "09_pilot", "10_demonstration",
    "11_industrial", "12_control", "13_hse_reliability", "14_tea_lca_ip", "15_qualification",
    "16_technology_package", "17_transfer", "18_field_learning", "data/raw", "data/processed",
    "models", "reports", "logs",
]


def bootstrap_project(brief: Path, output: Path, template_root: Path | None = None) -> dict:
    ...
    manifest = {
        ...
        "subskills": [x[0] for x in routed if x[0] in {"epdm", "poe", "polymer-general"}],
        "technical_approval_status": "NOT_EVALUATED",
        "gates": [
            {
                "gate_id": f"G{i}", "status": "NOT_EVALUATED", "owner": None,
                "evidence_ids": [], "approval_status": "NOT_EVALUATED", "approver": None,
            }
            for i in range(19)
        ],
    }
    (output / "project_manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
    (output / "brief.yaml").write_text(text, encoding="utf-8")
    return manifest


def audit_project(root: Path) -> list[str]:
    issues = [f"missing directory: {d}" for d in PROJECT_DIRS if not (root / d).is_dir()]
    ...
    if [g.get("gate_id") for g in data.get("gates", [])] != [f"G{i}" for i in range(19)]:
...
def deterministic_zip(root: Path, output: Path) -> str:
    ...
    files = sorted(
        ...
        if path.is_file() and not any(part in forbidden for part in path.relative_to(root).parts) and path.suffix not in {".pyc", ".pyo"}
    )
