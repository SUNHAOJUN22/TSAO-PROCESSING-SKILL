...
def _text(x: int, y: int, value: str, *, size: int = 16, color: str = TEXT, weight: int = 600, anchor: str = "start", mono: bool = False) -> str:
...
def _card(x: int, y: int, w: int, h: int, title: str, lines: tuple[str, ...], color: str) -> list[str]:
    body = [f'<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="18" fill="{SURFACE}" stroke="{color}" stroke-width="2"/>', _text(x + 22, y + 34, title, size=15, color=color, weight=800, mono=True)]
...
def render_canonical_publication_pipeline() -> str:
    p = _start("EPDM canonical publication pipeline", "Transactional strict JSON to immutable registry publication.")
    p += [_text(52, 72, "EPDM CANONICAL PUBLICATION", size=30, weight=850), _text(52, 108, "No partial typed state is published before every contract closes", size=15, color=MUTED)]
    stages = [
    ...
    p += _card(255, 430, 690, 130, "IMMUTABLE PUBLICATION", ("source_sha256 / registry_content_sha256 / publication_sha256", "failure at any stage exposes no partial registry"), GREEN)
    return _finish(p, "STRUCTURAL PASS DOES NOT OVERRIDE SCIENTIFIC OR HSE APPROVAL")


def render_governed_math_stack() -> str:
    p = _start("Governed mathematical stack", "Balances, kinetics, thermodynamics, numerics and inference with explicit Gates.")
    p += [_text(52, 72, "GOVERNED MATHEMATICAL STACK", size=30, weight=850), _text(52, 108, "Equation + units + domain + evidence + Gate", size=15, color=MUTED)]
    rows = [
...
def render_acceptance_readiness_map() -> str:
    p = _start("Software acceptance readiness map", "Separation of verified software evidence from external approvals.")
    p += [_text(52, 72, "SOFTWARE ACCEPTANCE READINESS", size=30, weight=850), _text(52, 108, "Software delivery can pass while external approval remains NOT_EVALUATED", size=15, color=MUTED)]
    p += _card(52, 190, 300, 120, "SOURCE IDENTITY", ("manifest / overlay", "SHA256 / byte count"), BLUE)
    p += _card(52, 350, 300, 120, "SOFTWARE GATES", ("tests / coverage / Ruff", "Doctor / Wheel / CI"), CYAN)
    p += _card(848, 190, 300, 120, "SCIENTIFIC", ("parameters / evidence", "applicability / uncertainty"), PURPLE)
    p += _card(848, 350, 300, 120, "ENGINEERING / HSE", ("equipment / relief", "HAZOP / LOPA / authority"), RED)
    p += [f'<rect x="430" y="210" width="340" height="260" rx="28" fill="{SURFACE}" stroke="{GREEN}" stroke-width="3"/>', _text(600, 275, "SOFTWARE DELIVERY", size=17, color=GREEN, weight=850, anchor="middle", mono=True), _text(600, 350, "PASS", size=56, color=GREEN, weight=900, anchor="middle"), _text(600, 405, "EXTERNAL: NOT_EVALUATED", size=16, color=AMBER, weight=800, anchor="middle", mono=True)]
