...
def inventory(root: Path, excluded: set[Path] | None = None) -> list[dict[str, object]]:
    ...
    for path in sorted(root.rglob("*")):
        ...
        if not path.is_file() or path.resolve() in excluded:
            continue
        rows.append({"path": path.relative_to(root).as_posix(), "bytes": path.stat().st_size, "extension": path.suffix.casefold(), "mime": mimetypes.guess_type(path.name)[0] or "", "sha256": sha256(path)})
    return rows
...
def main(argv: list[str] | None = None) -> int:
    ...
    with tsv_path.open("w", encoding="utf-8", newline="") as stream:
        writer = csv.DictWriter(stream, fieldnames=list(rows[0]) if rows else ["path", "bytes", "extension", "mime", "sha256"], delimiter="\t")
