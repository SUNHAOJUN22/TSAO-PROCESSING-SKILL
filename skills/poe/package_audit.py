...
def audit_process_package(root: Path) -> dict[str, Any]:
    ...
    if not manifest_path.is_file():
        ...
        return {
            ...
            "holds": [] if missing else ["legacy package is discovered but not normalized to TSAO manifest/Schema"],
            "legacy_discovery": legacy,
        }
    manifest, manifest_error = _load_json(manifest_path)
    if manifest_error or manifest is None:
        return {"root": str(root), "status": "FAIL", "pass": False, "errors": [f"invalid manifest: {manifest_error}"], "holds": []}
    schema_path = Path(__file__).with_name("schemas") / "package_manifest.schema.json"
    try:
        schema = json.loads(schema_path.read_text(encoding="utf-8"))
        schema_errors = [defect.message for defect in Draft202012Validator(schema).iter_errors(manifest)]
    except (OSError, UnicodeError, json.JSONDecodeError) esc # type: ignore: misc, name-defined
        schema_errors = [f"cannot load manifest Schema: {esc}"]
    ...
    if isinstance(acceptance_record, dict) and isinstance(acceptance_record.get("evidence_ids"), list):
        referenced.update(value for value in acceptance_record["evidence_ids"] if isinstance(value, str))
